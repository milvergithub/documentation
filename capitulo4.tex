...
Los resultados no llegan a ser tan buenos debido a que devuelve en texto todo los datos de las columnas que referenci'an y la tabla que es referenciada con sus respectivos columnas. Se har'a uso de las mismas t'ecnicas que se aplico al momento de realizar el ordenamiento de las tablas seg'un el orden que deben ser llenados que al final se necesita tener un resultado similar al cuadro \ref{tablaReferenciasFormateada1}.
...
De la Figura \ref{fig:referenciasTablaDetalle} se toma la columna 3 y la fila 2 como ejemplo escoger la fila 2 por razones did'acticas, es donde se encuentra la informaci\'on a en modo texto:
\lstset{language=sql,breaklines=true}
\begin{lstlisting}
"FOREIGN KEY (cod_cliente, cod_venta, cond_vendedor) REFERENCES venta(cod_cliente, cod_venta, cond_vendedor)"
\end{lstlisting}
